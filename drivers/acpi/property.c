...
static int software_node_read_u32_prop_index(const struct fwnode_handle *fwnode,
					     const char *propname,
				      ...
				      unsigned int index)
{
	...
	ret = software_node_read_u32_prop_index(fwnode, ACPI_IRQ_PROP_GSI,
						index, &gsi);
	if (ret)
		return ret == -ENOENT ? -ENXIO : ret;

	ret = software_node_read_u32_prop_index(fwnode, ACPI_IRQ_PROP_GSI_TRIGGER,
						index, &trigger);
	if (ret)
		return ret == -ENOENT ? -ENXIO : ret;

	ret = software_node_read_u32_prop_index(fwnode, ACPI_IRQ_PROP_GSI_POLARITY,
						index, &polarity);
	if (ret)
		return ret == -ENOENT ? -ENXIO : ret;
