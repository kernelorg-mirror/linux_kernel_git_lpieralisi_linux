...
static void __init acpi_iort_add_irq(u32 hwirq, const char *name, int trigger_val,
				     u32 *gsi, u32 *trigger,
				     ...
				     int *irq_count)
{
	...
	gsi[*irq_count] = hwirq;
	trigger[*irq_count] = trigger_val;
	polarity[*irq_count] = ACPI_ACTIVE_HIGH;
	if (names)
		names[*irq_count] = name;
	(*irq_count)++;
}

static int __init acpi_iort_dup_irq_props(const u32 *gsi,
					  const u32 *trigger,
					  const u32 *polarity,
					     ...
					     struct acpi_iort_node *node)
{
	...
	return acpi_iort_dup_irq_props(gsi, trigger, polarity, names,
				       irq_count, props);
}
					  ...
					  struct acpi_iort_node *node)
{
	...
	u64 *ctx_irq, *glb_irq;
	int i, irq_count = 0, ret = -ENOMEM;

	smmu = (struct acpi_iort_smmu *)node->node_data;

	gsi = kcalloc(smmu->context_interrupt_count + 1, sizeof(*gsi),
		      GFP_KERNEL);
	trigger = kcalloc(smmu->context_interrupt_count + 1,
			     sizeof(*trigger), GFP_KERNEL);
	polarity = kcalloc(smmu->context_interrupt_count + 1,
	...
	ret = acpi_iort_dup_irq_props(gsi, trigger, polarity, NULL,
				      irq_count, props);
