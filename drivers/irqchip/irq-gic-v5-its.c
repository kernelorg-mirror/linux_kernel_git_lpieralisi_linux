...
static struct acpi_madt_gicv5_translator *current_its_entry;
static struct fwnode_handle *current_its_fwnode;
						    ...
						    const unsigned long end)
{
	...
	err = iort_register_domain_token(its_frame->translate_frame_id, res.start,
					 msi_dom_handle);
	if (err) {
		pr_err("ITS@%pa: Unable to register GICv5 ITS domain token (ITS ID %d) to IORT\n",
